...
Расширение для `edvibe.com`, которое добавляет ссылку на Sõnaveeb для текущего слова и показывает формы из Sonapi.

## Для пользователей (без разработки)

### Вариант 1: через Chrome Web Store (рекомендуется)

[Установить из Chrome Web Store](https://chromewebstore.google.com/detail/edvibe-sonaveeb-link/cnakfbmdahlblhjbmahheogefpfliian)

1. Откройте страницу расширения в Chrome Web Store.
2. Нажмите `Установить`.
3. Перейдите на страницу слов в Edvibe и обновите вкладку.

### Вариант 2: вручную из исходников (для локальной проверки)

1. Скачайте исходники проекта (или архив с GitHub и распакуйте его).
...
- Запрашивает формы из `https://api.sonapi.ee/v2/<word>` и показывает их рядом с кнопкой.
  - Для существительных используются: `SgN`, `SgG`, `SgP`, `PlP`.
  - Для глаголов используются: базовая форма, `Inf`, `IndPrSg1` (с запасным `SupIps`).
- Если в ответе Sonapi есть данные, показывает один короткий пример предложения; если примеров несколько, выбирает случайный.
- Для глаголов дополнительно показывает `Rektsioon`, если он указан в Sonapi.
- Пример предложения можно автоматически копировать в буфер обмена через настройку расширения.
...
- Положение блока: сверху или снизу от карточки.
- Отключение автоозвучки карточек (опция по умолчанию включена).
- Автоматическое копирование примера предложения в буфер обмена (по умолчанию выключено).

Значения по умолчанию:
- Положение блока: сверху по центру (`top`).
- Автоозвучка: отключается расширением (`disableAutoplay = true`).
- Автокопирование примера: выключено (`autoCopyExample = false`).
